...
function LevelService.LavaLine(level, data)
    local rlp = randomLevelPoint(level)
    if rlp then
        ...
        local properties = {Time = data.fadeInTime}
        TweenService.tween(lava, goal, properties)

        task.wait(data.fadeInTime)
    ...
    for key, data in pairs(ChanceData) do
        ...
        if rng:NextInteger(1, chance) <= 1 + (playerLuck / 10) then
            task.spawn(function()
                ...
                elseif key == "LavaLine" then
                    LevelService.LavaLine(level, data)
                end
            end)
